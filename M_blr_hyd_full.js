...
function enableSmartPaste() {
  ...
  allRows.forEach((row, rowIndex) => {
    ...
    const inputs = row.querySelectorAll("input.summary-input");
    if (!inputs.length) return;

    // Attach paste only to FIRST manual column (Actual Riders)
    inputs[0].addEventListener("paste", function (e) {
      ...
      rows.forEach((rowText, pasteRowIndex) => {

        const cols = rowText.split(/\t/); // split by TAB
        const targetRow = allRows[pasteRowIndex];
        if (!targetRow) return;

        const targetInputs = targetRow.querySelectorAll("input.summary-input");

        cols.forEach((value, colIndex) => {
          if (targetInputs[colIndex]) {
            targetInputs[colIndex].value = value.trim();
          }
        });
