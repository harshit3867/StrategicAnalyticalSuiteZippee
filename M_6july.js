...
const CITY_STORES = {
  bangalore: STORE_ORDER.slice(0, 7),
  hyderabad: STORE_ORDER.slice(7)
};
  ...
  fetch(PROJECTION_CSV_URL)
    ...
    .then(csvText => {
      ...
      reportData = rows.map(row => {
        ...
        const storeColumns = Object.keys(row).slice(1, 12); // B → M
...
">
  ...
  /* =========================
     TOTAL ROW
  ========================= */
  const totalAttainment = sumProjTill ? ((sumTotalOrders / sumProjTill) * 100).toFixed(2) : "0.00";
  const totalRow = document.createElement("tr");
